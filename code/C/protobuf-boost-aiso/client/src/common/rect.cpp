...
std::istream& operator>>(std::istream& in, kydroid::graphics::Rect &rect)
try {
  std::string str;
  in >> str;
  auto tokens = kydroid::utils::string_split(str, ',');

  switch (tokens.size()) {
  case 2: {
    rect = kydroid::graphics::Rect(0, 0,
             boost::lexical_cast<std::int32_t>(tokens[0]),
             boost::lexical_cast<std::int32_t>(tokens[1]));
    break;
  }
  case 4:
    rect = kydroid::graphics::Rect(
             boost::lexical_cast<std::int32_t>(tokens[0]),
             boost::lexical_cast<std::int32_t>(tokens[1]),
