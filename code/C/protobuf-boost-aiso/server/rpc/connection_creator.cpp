...
//2019 读取kydroidd发送过来的数据   boost::asio::local::stream_protocol::socket本地连接
void ConnectionCreator::create_connection_for(
    std::shared_ptr<boost::asio::local::stream_protocol::socket> const&
        socket) {

    if (connections_->size() >= 1) {
        printf("A second client tried to connect. Clear the old clients and only allow a single client\n");
        connections_->clear();
    }
