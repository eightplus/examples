...
std::string remove_socket_if_stale(std::string const& socket_name) {
    if (socket_file_exists(socket_name) && socket_exists(socket_name)) {
        //terminate called after throwing an instance of 'boost::exception_detail::clone_impl<boost::exception_detail::error_info_injector<boost::system::system_error> >' what():  bind: 地址已在使用
        //char buffer[256];
        //snprintf(buffer, sizeof(buffer), "[Function:%s Line:%d]bind: 地址已在使用!", __func__, __LINE__);
        printf("bind: 地址已在使用\n");
    }
