...
HostConnector::HostConnector() :
    socket_(std::make_shared<LocalSocketConnection>("/tmp/my_desktop")),
...
void HostConnector::main_loop() {
    while (running_) {
        ...
        if (!message_processor_->process_data(data))//读取server端通过callmethod发送过来的数据
            break;
    }
}

std::shared_ptr<kydroid::PlatformApiStub> HostConnector::platform_api_stub() const {
